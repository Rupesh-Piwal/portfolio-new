...
export function AnimatedTextSlider({
  ...
  return (
    <div
      className={`relative h-[1.5em] overflow-hidden w-full font-thin ${className}`}
    >
      {texts.map((text, i) => {
        ...
        return (
          <div
            key={i}
            className={`absolute left-0 w-full font-thin ${transformClass} ${opacityClass} ${shouldTransition ? "transition-all duration-700 ease-in-out" : ""}`}
          >
            {text}
          </div>
        );
